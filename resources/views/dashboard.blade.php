<x-dashboard>
    <main class="main-container">
        ...
        <div class="main-cards">
          ...
          <div class="card">
            ...
            <span class="text-primary font-weight-bold">{{ $numeroUsuarios }}</span>
          </div>

          <div class="card">
            <div class="card-inner">
              <p class="text-primary">PUBLICACIONES</p>
              <span class="material-icons-outlined text-orange">mark_as_unread</span>
            </div>
            <span class="text-primary font-weight-bold">{{ $numeroPublicaciones }}</span>
          </div>
          ...
          <div class="card">
            ...
            <span class="text-primary font-weight-bold">{{ $numeroComentarios }}</span>
          </div>
          ...
          <div class="card">
            ...
            <span class="text-primary font-weight-bold">{{ $numeroCarreras }}</span>
          </div>
    ...
    <script>
        ...
        var datosUsuariosPorMes = {
            labels: {!! isset($usuariosPorMes) ? json_encode($usuariosPorMes->pluck('month')->map(function($month) {
                return date('F', mktime(0, 0, 0, $month, 1));
            })) : '[]' !!},
            datasets: [{
                label: 'Cantidad de Usuarios Registrados',
                data: {!! isset($usuariosPorMes) ? json_encode($usuariosPorMes->pluck('total')) : '[]' !!},
